...
from multiprocess import Process, Queue, freeze_support
...
processes = [Process(target=multi_comparer,args=(lines[0],lines[(i*lines_len//cpu):(i+1)*lines_len//cpu],q)) for i in range(cpu)]
for pr in processes:
  pr.start()
